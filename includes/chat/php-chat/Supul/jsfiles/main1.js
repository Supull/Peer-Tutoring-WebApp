...
document.addEventListener('click', function(event) {
    if (event.target.classList.contains('cancel-button')) {
        const requestId = event.target.getAttribute('data-request-id');
        const requester = event.target.getAttribute('data-requester') || ''; // Handle requester's screen
        const tutor = event.target.getAttribute('data-tutor') || ''; // Handle tutor's screen

        fetch('includes2/cancel_request.php', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ requestId: requestId, requester: requester, tutor: tutor })
        })
    ...
    if (event.target.classList.contains('accept-button')) {
        ...
        fetch('includes2/accept_request.php', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
...
function loadUsername() {
    fetch('includes2/get_username.php') 
    .then(response => response.json())
    .then(data => {
        if (data.username) {
            document.getElementById('usernameDisplay').innerText = `${data.username}`;
        } else {
            document.getElementById('usernameDisplay').innerText = 'Username not available';
        }
    })
    .catch(error => {
...
fetch('includes2/get_online_users.php?subject=' + encodeURIComponent(selectedSubject))
.then(response => response.text())
.then(data => {
...
function loadIncomingRequests() {
    fetch('includes2/get_incoming_requests.php')
    .then(response => response.text())
    .then(data => {
        document.getElementById('incomingRequests').innerHTML = data;
    })
    .catch(error => {
...
function loadAcceptedRequests() {
    fetch('includes2/get_accepted_requests.php')
    .then(response => response.text())
    .then(data => {
...
document.addEventListener('click', function(event) {
    if (event.target.classList.contains('request-button')) {
        const tutorUsername = event.target.getAttribute('data-tutor');
        const button = event.target;

        button.disabled = true;

        fetch('includes2/send_request.php', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ tutor: tutorUsername })
        })
        .then(response => response.text())
        .then(result => {
            alert(result);
